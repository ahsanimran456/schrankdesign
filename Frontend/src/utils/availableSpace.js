...
export const getBottom = (positionY, scaleY, type, topShelfDistance) => {
  ...
  if (type === Config.furnishing.type.drawer) {
    bottom += topShelfDistance + Config.furnishing.drawer.shelfThickness1
  } else if (type === Config.furnishing.type.internalDrawer) {
    bottom +=
      Config.furnishing.internalDrawer.topShelfDistance +
      Config.furnishing.drawer.shelfThickness1
...
const getSpace = (
  ...
  for (let i = 0; i < elementsXInfo.length; i++) {
    if (assets[i] === undefined || assets[i].length === 0) {
      ...
      if (tempSpace.top > tempSpace.bottom) result.push(tempSpace)
    } else {
      ...
      for (let j = 0; j < assets[i].length; j++) {
              ...
              case Config.furnishing.type.drawer:
                if (
                  ...
                  if (
                    ...
                    bottom = bottom - Config.furnishing.drawer.shelfThickness1
                  } else {
        ...
        if (top > bottom) result.push(tempSpace)
      }
...
export const getDoorSpace = (
  ...
  const assets = [
    ...furnishingAssets.filter(
      (asset) =>
        !asset.inDivider && asset.type === Config.furnishing.type.drawer
    ),
    ...doorAssets.filter((asset) => !asset.selected),
  ].sort((a, b) => {
...
export const getAvailableSpace = (
  ...
  furnishingAssets.forEach((item) => {
    if (item.inDivider && !item.selected) {
  ...
  const f_assets = furnishingAssets
    .filter((asset) => !asset.inDivider && !asset.selected)
    .sort((a, b) => {
      if (a.xIndex !== b.xIndex) {
        return a.xIndex - b.xIndex
      }
